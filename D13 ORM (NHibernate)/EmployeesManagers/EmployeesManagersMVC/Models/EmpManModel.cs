...
namespace EmployeesManagersMVC.Models
{
    ...
    public class EMList
    {
        public static EmpManModel Find(int id)
        {
            ...
            Employee _temp2 = Store.FindEmployee(id);
            if (_temp2 != null)
            {
        ...
        public static List<EmpManModel> List
        {
            get
            {
                ...
                foreach(Employee emp in Store.EmpList)
                {
                    _temp = new EmpManModel();
                    _temp.Id = emp.Id;
        ...
        public static void Add(EmpManModel _employee)
        {
            ...
            if (_employee.ManagerId > 0)
                E.Manager = Store.FindEmployee(_employee.ManagerId);
            if (_employee.CityId > 0)
                E.City = Store.FindCity(_employee.CityId);
            Store.AddOrUpdate(E);
        }

        public static void Update(EmpManModel _employee)
        {
            Employee E = Store.FindEmployee(_employee.Id);
            E.FirstName = _employee.FirstName;
            E.LastName = _employee.LastName;
            E.Salary = _employee.Salary;
            if (_employee.ManagerId > 0)
    ...
    public class EList
    {
        public static List<EmployeeName> List(int eid = 0)
        {
            ...
            foreach (Employee E in Store.EmpList)
            {
                if (E.Id == eid)
                    continue;
    ...
    public class CList
    {
        public static List<City> List
        {
            get
            {
                ...
                foreach (City city in Store.CitiesList)
                    _list.Add(city);
                return _list;
            }
        } 
